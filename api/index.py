...
# Custom Curated Niche Categories definition
CATEGORIES = {
    "teen-horror": {
        "name": "Teen Horror",
        "movie": "with_genres=27&with_keywords=11876",
        "tv": "with_genres=9648,18,80&with_keywords=11876"
    },
    "raw-teen-terror": {
        "name": "Raw Teen Terror (18+)",
        "movie": "with_genres=27&with_keywords=11876&certification_country=US&certification=R",
        "tv": "with_genres=27,9648&with_keywords=11876&certification_country=US&certification=TV-MA"
    },
    "apocalypse": {
        "name": "Apocalypse & Survival",
        "movie": "with_keywords=4565|2853|10349|4563",
        "tv": "with_keywords=4565|2853|10349|4563"
    },
    "slasher-suspense": {
        "name": "Slasher Suspense",
        "movie": "with_genres=27,53&with_keywords=12339",
        "tv": "with_genres=80,53,27&with_keywords=12339"
    },
    "sibling-bonds": {
        "name": "Sibling Bonds",
        "movie": "with_keywords=11599|6149|5638",
        "tv": "with_keywords=11599|6149|5638"
    },
    "pure-cinema": {
        "name": "Pure Cinema",
        "movie": "sort_by=vote_average.desc&vote_count.gte=10000",
        "tv": "sort_by=vote_average.desc&vote_count.gte=2500"
    },
    "erotic-raw": {
        "name": "Erotic & Raw",
        "movie": "with_keywords=18428|191726|155395|41258",
        "tv": "with_keywords=18428|191726|155395|41258"
    },
    "hidden-gems": {
        "name": "Hidden Gems",
        "movie": "with_genres=27,53,9648&vote_average.gte=6.8&vote_count.gte=100&vote_count.lte=1500&sort_by=vote_average.desc",
        "tv": "with_genres=27,53,9648&vote_average.gte=6.8&vote_count.gte=40&vote_count.lte=1000&sort_by=vote_average.desc"
    }
}

@app.route('/api/custom-category/<category_id>')
def custom_category(category_id):
    client_page = request.args.get('page', type=int)
    media_type = request.args.get('type', 'movie')
    
    cfg = CATEGORIES.get(category_id)
    if not cfg:
        return jsonify({'error': 'Category not found'}), 404
        
    query_params = cfg.get(media_type, "")
    
    if media_type == 'tv':
        base_url = f"https://api.tmdb.org/3/discover/tv?api_key={TMDB_API_KEY}&{query_params}"
        filter_fn = anime_filter
    else:
        base_url = f"https://api.tmdb.org/3/discover/movie?api_key={TMDB_API_KEY}&{query_params}"
        filter_fn = None
        
    try:
        data = fetch_and_filter_tmdb(base_url, client_page, filter_fn)
        data['name'] = cfg['name']
        return jsonify(data)
    except Exception as e:
        return jsonify({'error': str(e)}), 500
